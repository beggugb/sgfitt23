...
const ComponentToPrint = React.forwardRef((props,ref)=>{    
    ...
    return ( 
        ...
        <div className="h-9 text-center mt-8">
            <h6 className="font-bold">Informe de Ventas</h6>                                                           
            <h6>
            ...
            <table className='border-collapse w-full'>
                <thead>
                    <tr className='h-7 border bg-gray-100'>                                            
                        ...
                        <th className='w-3/12 border'>Producto</th>                                                                                         
                        <th className='w-2/12 border'>P.Unitario</th>    
                        <th className='w-1/12 border'>Cantidad</th>
                        <th className='w-2/12 border'>SubTotal</th>                        
                ...
                <tbody>
                    { props.pdata.map((it,index)=>(
                        <tr key={index} >
                            <td className="border text-center"><Moment format="DD/MM/YYYY">{it.fechaVenta}</Moment></td>                                
                            <td className='border pl-1'>{it.nombre}</td>
                            <td className="border text-center">
                            {new Intl.NumberFormat('es-BO',{minimumFractionDigits: 2}).format(it.valor)}</td>
                            <td className='border text-center'>{it.cantidad}</td>                                                                           
                            <td className="border text-center">
                            {new Intl.NumberFormat('es-BO',{minimumFractionDigits: 2}).format(it.total)}</td>
                            <td className="border text-center">{it.usuario ? it.usuario : 'n'}</td>
...
const Ventas = () => {    
    ...
    const {detalle, ventas, desde, hasta } = useSelector(state => state.informes)
